...
class Agent():
    ...
    def __init__(self, config):
        ...
        """
        print("[AGENT INFO] DDPG constructor initialized parameters:\n num_agents={} \n state_size={} \n action_size={} \n random_seed={} \n actor_fc1_units={} \n actor_fc2_units={} \n critic_fcs1_units={} \n critic_fc2_units={} \n buffer_size={} \n batch_size={} \n gamma={} \n tau={} \n lr_actor={} \n lr_critic={} \n weight_decay={} \n ou_mu={}\n ou_theta={}\n ou_sigma={}\n update_every_t_steps={}\n num_of_updates={}\n".format(self.config.num_agents, self.config.state_size, self.config.action_size, self.config.random_seed, self.config.actor_fc1_units, self.config.actor_fc2_units, self.config.critic_fcs1_units, self.config.critic_fc2_units, self.config.buffer_size, self.config.batch_size, self.config.gamma, self.config.tau, self.config.lr_actor, self.config.lr_critic, self.config.weight_decay, self.config.ou_mu, self.config.ou_theta, self.config.ou_sigma, self.config.update_every_t_steps, self.config.num_of_updates))
        
        # Actor Network (w/ Target Network)
        self.actor_local = Actor(self.config.state_size, self.config.action_size, self.config.random_seed, self.config.actor_fc1_units, self.config.actor_fc2_units).to(device)
        self.actor_target = Actor(self.config.state_size, self.config.action_size, self.config.random_seed, self.config.actor_fc1_units, self.config.actor_fc2_units).to(device)
        self.actor_optimizer = optim.Adam(self.actor_local.parameters(), lr=self.config.lr_actor)

        # Critic Network (w/ Target Network)
        self.critic_local = Critic(self.config.state_size, self.config.action_size, self.config.random_seed, self.config.critic_fcs1_units, self.config.critic_fc2_units).to(device)
        self.critic_target = Critic(self.config.state_size, self.config.action_size, self.config.random_seed, self.config.critic_fcs1_units, self.config.critic_fc2_units).to(device)
        ...
        # Noise process
        self.noise = OUNoise(self.config.action_size, self.config.random_seed, mu=self.config.ou_mu, theta=self.config.ou_theta, sigma=self.config.ou_sigma)
